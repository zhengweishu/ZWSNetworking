...
{
    static ZWSAppContext *sharedInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[ZWSAppContext alloc] init];
...
- (CGFloat)latitude
{
    return [ZWSLocationManager sharedInstance].currentLocation.coordinate.latitude;
}

- (CGFloat)longitude
{
    return [ZWSLocationManager sharedInstance].currentLocation.coordinate.longitude;
}

- (void)appStarted
{
    self.sessionId = [[NSUUID UUID].UUIDString copy];
    [[ZWSLocationManager sharedInstance] startLocation];
}

- (void)appEnded
{
    [[ZWSLocationManager sharedInstance] stopLocation];
}
...
{
    BOOL isOnline = NO;
    NSString *filepath = [[NSBundle mainBundle] pathForResource:@"ZWSNetworkingConfiguration" ofType:@"plist"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath]) {
        NSDictionary *settings = [[NSDictionary alloc] initWithContentsOfFile:filepath];
        isOnline = [settings[@"isOnline"] boolValue];
    } else {
        isOnline = kZWSServiceIsOnline;
    }
    return isOnline;
}
