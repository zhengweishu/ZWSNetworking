...
- (NSString *)zws_uuid
{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self zws_createUUID] forKey:key];
...
- (NSString *)zws_udid
{
    NSString *udid = [[ZWSUDIDGenerator sharedInstance] UDID];
    if (udid.length==0) {
        udid = [self zws_uuid];
        [[ZWSUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *)zws_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice]zws_macaddress];
        macid = [macaddress zws_md5];
        if (!macid){
